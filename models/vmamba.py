...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # a = self.out_proj(y)
        # a = self.effn(y)
        # print("self.out_proj(y)", a.shape)
        out = self.dropout(self.effn(y))
        return out
...
class EFFN(nn.Module):
    def __init__(self, inch):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=inch, out_channels=inch*2, kernel_size=1, padding=0),
            nn.BatchNorm2d(num_features=inch*2),
            nn.ReLU()
        )
        # dw卷积，groups=input_channel
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=inch*2, out_channels=inch*2, kernel_size=3, padding=1, groups=inch*2),
            nn.BatchNorm2d(num_features=inch*2),
            nn.ReLU()
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=inch*2, out_channels=inch//2, kernel_size=1, padding=0),
            # nn.BatchNorm2d(num_features=17),
            # nn.ReLU()
        )
        self.se = SE_Block(inch//2)
...
def check_vssm1_equals_vssm(forward_type="v0"):
    ...
    VSSM1 = partial(VSSM, downsample_version="v1", patchembed_version="v1", mlp_ratio=0.0, ssm_ratio=2.0,
                    ssm_rank_ratio=2.0, forward_type=forward_type)
    VSSM.forward_backbone = VSSM_.forward_backbone
    VSSM.forward1 = VSSM_.forward1
    # expected to be all the same 
    VSSM1 = partial(VSSM_, downsample_version="none", patchembed_version="v1", mlp_ratio=0.0, ssm_ratio=2.0,
                    ssm_rank_ratio=2.0, forward_type=forward_type)
